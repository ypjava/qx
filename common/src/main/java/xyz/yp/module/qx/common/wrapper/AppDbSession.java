...
import com.rongdu.cashloan.rc.domain.Merchant;
...
public class AppDbSession {
	...
	public AppSessionBean create(Merchant merchant) {
		String token = UUID.randomUUID().toString().replaceAll("-", "");
		String refreshToken = merchant.getUuid();
		...
		Map appSession = dbService.queryRec("select * from cl_app_session where user_id=" + merchant.getId() + " order by id desc limit 1");
		if (appSession != null) {
			getCache().evict(appSession.get("token"));
			dbService.update(SqlUtil.buildUpdateSql(
					"cl_app_session",
					MapUtil.array2Map(new Object[][] {
							{ "id", appSession.get("id") },
							{ "status", 0 },
							{"err_data", JsonUtil.toString(MapUtil
